...
# Abuznaid : generate n_stops of random numbers for both x and y
    ...
    def render(self, i, return_img=False):
        ...
        if return_img:
            ...
            # print("type of fig out of render", type(fig))
            # canvas = FigureCanvasTkAgg(fig)
            # canvas.draw()
            # canvas.get_tk_widget().pack(side='top', fill='both', expand=1)
            # image = np.frombuffer(canvas.tostring_rgb(), dtype=np.uint8)
            # print ("type of images out of render" , type(image))

# trying to save the figures first then convert it to gif
#             filename = f"figures/figure_{self.i + 1}.png"
#             fig.savefig(filename)
#
#             # open the png file as an RGBA image
#             img_rgba = Image.open(filename)
#
#             # convert the RGBA image to RGB format
#             img_rgb = img_rgba.convert('RGB')
#
#             # save the RGB image as a new png file
#             new_filename = filename.replace('.png', '_rgb.png')
#             img_rgb.save(new_filename)
#
#             # append the RGB image to the images list
#             img_array = imageio.imread(new_filename)
#
#             # delete the temporary RGB image file
...
def run_n_episodes(env,agent,name="training_abuzniad.gif",n_episodes=200,render_each=1,fps=10):
